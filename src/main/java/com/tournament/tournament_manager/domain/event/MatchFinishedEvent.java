...
public record MatchFinishedEvent(Long matchId, int player1EloBefore, int player2EloBefore) {
    ...
    /**
     * Constructeur de convenance pour un bye : aucune probabilité de victoire n'a de sens
     * sans second joueur.
     *
     * <p>{@code @JsonIgnore} est nécessaire ici : sans lui, Jackson détecte deux
     * constructeurs candidats pour la désérialisation (celui-ci et le constructeur
     * canonique) et échoue avec {@code InvalidDefinitionException: Conflicting
     * property-based creators} — un problème connu de Jackson avec les records ayant
     * plusieurs constructeurs (FasterXML/jackson-databind#3968).
     */
    @JsonIgnore
    public MatchFinishedEvent(Long matchId) {
        this(matchId, 0, 0);
    }
}
